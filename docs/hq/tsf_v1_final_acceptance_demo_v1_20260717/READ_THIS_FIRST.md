# TSF V1 Final Acceptance, Demo, and Release Candidate V1

This packet is the Milestone 4 acceptance layer for the already-built Thousand Sunny Fleet V1. It does not replace the canonical mission, queue, lifecycle, verifier, admission, approval, replay, or recovery authorities.

Required baseline: `952f30e137214735fe2513a7b068d9680ca882c7`.

The source changes are intentionally narrow:

- the human Doctor formatter uses each authoritative check `id`, and rejects a missing label;
- parser evidence records an honest numeric exit and explicit parser-result identity for every row;
- the accepted M3 validation hash typo is preserved and corrected by an additive erratum;
- the Start/Doctor/Stop test waits boundedly for the demo banner it asserts, removing a stdout scheduling race;
- one repeatable M4 runner composes accepted milestone and canonical suites, the bounded real app-server proof, cleanup assertions, and hashed evidence.

The deterministic Demo is clearly labeled fixture behavior. The final acceptance run is what proves the real Codex app-server interruption and distinct new-run recovery path under `CODEX_SERVICE_ONLY`, with worker-tool network disabled.

The current sealed candidate `f1a9fc6d64add5370067b90f2cae668bf55f342c` passed all 21 acceptance gates in 804.511 seconds. The real proof and its authoritative PASS basis both record 94 assertions, final Doctor was safely governed `TIM_REQUIRED`, and cleanup found no owner, listener, or owned child. The first and second independent audits were RED and are preserved in [INDEPENDENT_AUDIT_RED_1.md](INDEPENDENT_AUDIT_RED_1.md) and [INDEPENDENT_AUDIT_RED_2.md](INDEPENDENT_AUDIT_RED_2.md); publication remains blocked until a fresh audit is GREEN. See [ACCEPTANCE_SEAL.json](ACCEPTANCE_SEAL.json), [VALIDATION.md](VALIDATION.md), [REAL_APP_SERVER_PROOF.json](REAL_APP_SERVER_PROOF.json), and [PRE_GREEN_FAILURE_DISPOSITION.md](PRE_GREEN_FAILURE_DISPOSITION.md).

The fresh independent Tester and Auditor both reported GREEN with no findings on frozen candidate `e9d29bfb79e3b4a3da48b11887fa7a0bd8a9090e`; [INDEPENDENT_AUDIT_GREEN_FINAL.md](INDEPENDENT_AUDIT_GREEN_FINAL.md) records the disposition. They authorize one normal non-force push and one ready PR. This milestone never merges or enables auto-merge.

Start with [OPERATOR_RUNBOOK.md](OPERATOR_RUNBOOK.md), [DEMO_SCRIPT.md](DEMO_SCRIPT.md), and [PHASE_0_FINAL_INVENTORY.md](PHASE_0_FINAL_INVENTORY.md). The exact executed rows and raw-evidence hashes are in [EXECUTED_TEST_COVERAGE.csv](EXECUTED_TEST_COVERAGE.csv) and [EVIDENCE_INDEX.csv](EVIDENCE_INDEX.csv).
