...
This is nonblocking because only a copied validation field is wrong. The adoption manifest, status bytes, archive, patches, classification, and accepted M3 source are not changed. The machine-readable binding and eventual M4 auditor disposition are in [M3_VALIDATION_ERRATA_V1.json](M3_VALIDATION_ERRATA_V1.json).
