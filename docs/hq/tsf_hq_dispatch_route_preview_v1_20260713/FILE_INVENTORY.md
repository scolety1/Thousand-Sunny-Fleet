...
| `fleet/control/hq-dispatch/hq-dispatch-route-preview-response.schema.v1.json` | Closed preview response and artifact schema. |
...
| Path | Purpose |
| --- | --- |
| `tools/hq-dispatch/v1/server.mjs` | Fixed loopback-only Node HTTP server and sole child-process invocation site. |
| `tools/hq-dispatch/v1/Invoke-TsfHqDispatchRoutePreview.ps1` | Stdin-only, no-argument canonical route-preview wrapper with fixed sources and output root. |
| `tools/hq-dispatch/v1/public/index.html` | Operator shell with the exact preview-only banner and one Preview route control. |
...
| Path | Purpose |
| --- | --- |
| `tests/test-tsf-hq-dispatch-route-preview-v1.mjs` | 92-assertion foreground endpoint, injection, source-parity, UI-control, and mutation-snapshot suite. |
| `tests/run-tsf-hq-dispatch-route-preview-v1-tests.ps1` | 216-assertion schema, hash, static-plugin, parser, protected-source, scope, and integration harness. |
...
| `docs/hq/tsf_hq_dispatch_route_preview_v1_20260713/SHA256SUMS.txt` | SHA-256 hashes for the other 19 intended files; the checksum file does not hash itself. |
