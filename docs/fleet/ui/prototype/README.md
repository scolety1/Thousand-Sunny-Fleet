# Fleet Console Static Mock Prototype V3 + Session Handoff V1
...
Session Handoff V1 adds `fleet/status/return-review.md`, generated by
`tools/write-return-review.ps1` from TSF-local status/docs/fixture inputs. It is
the calm one-page answer for "what do I need to look at first?" when the console
itself feels like too much. It answers top recommendation, Needs Tim, Ready to
approve, Done while away, Blocked / unsafe, Next best work session, and
Suggested next Codex prompt.

Tim only has to remember:

1. Open Fleet Console.
2. Read "What do I do now?"
3. Check `fleet/status/return-review.md` if needed.
4. Copy a work order.
5. Send it to Codex.
...
The Work Order Library is copy/paste text only. Work Order Composer V1 includes
templates for start work while Tim is here, busy mode work, away-safe work, deep
research/root file intake, project onboarding, return review after being gone,
commit-ready review, and push approval review.

Each template carries the same fields: project name placeholder, repo path
placeholder, goal, files/artifacts, off-limits, autonomy/availability mode, stop
conditions, validation expectations, and final report format.
...
The read-only return-review helper writes only `fleet/status/return-review.md`.
It reads the same TSF-local status and console sources, plus project-management
