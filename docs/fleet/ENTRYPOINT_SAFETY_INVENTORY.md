...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `invoke-autonomy-wrapper.ps1` | `selected_ship_required` | yes, selected ship state/repo status outside runtime pilot dry-run | limited harness actions only when `-Execute` and explicit `-Allow*` switches are present; `-RuntimePolicyPilotDryRun` never executes product actions | exactly one `-Ship` or `-Preset fixture-only`; default `MaxShips = 1`; runtime pilot accepts only schema-shaped dry-run evidence | no product launcher in current wrapper; may call audit packaging when allowed outside runtime pilot dry-run | usually yes | do not run implementation actions under low budget; runtime pilot remains evidence-only | bounded Stage 8 wrapper plus dry-run runtime pilot | Preferred autonomy-facing wrapper, not a broad launcher. Runtime pilot reports `executesProductActions = false`. |
...
- Runtime pilot use of `invoke-autonomy-wrapper.ps1 -RuntimePolicyPilotDryRun` is evidence-only for one named entrypoint. It must report `executesProductActions = false`, `launchesShips = false`, `importsPackets = false`, and `mutatesProductRepos = false`, even if `-Execute` is present.
- Wrapper source visibility for `invoke-autonomy-wrapper.ps1` is audit inspection only. It may be included in a bounded runtime pilot audit package, or represented by a human-reviewed checksum/source excerpt, so reviewers can inspect the `-RuntimePolicyPilotDryRun` early-exit path. Source visibility does not approve execution, package sending, product repo access, runtime widening, staging, commit, push, or a demo trial.
