...
25. For the post-combined GREEN follow-up audit milestone, use `docs/fleet/POST_COMBINED_GREEN_FOLLOWUP_AUDIT_RECORD_2026_06_04.md` as evidence only. It records that `C:\Users\codex-agent\Downloads\Codex Fleet Audit (7).docx` returned GREEN for the post-combined follow-up hardening package, with INFO-only optional queue candidates. It does not approve product-repo access, product mutation, real demo execution, package creation or sending, runtime command binding, remote access, phone approvals, all-fleet commands, overnight runner execution, non-mock UI implementation, staging, commit, push, deploy, installs, migrations, secrets/auth/payments/deploy work, lock deletion, permission widening, or future authority.
26. For post-combined optional INFO hardening, use the `Post-Combined GREEN Optional INFO Hardening Queue 2026-06-04` section in `docs/fleet/HQ_REPAIR_TASK_QUEUE.md` and the refreshed optional INFO audit sections in `docs/fleet/HQ_NEXT_EXTERNAL_AUDIT_PROMPT.md`, `docs/fleet/READ_ONLY_DEMO_READINESS_EXTERNAL_AUDIT_PROMPT.md`, and `docs/fleet/EXTERNAL_AUDIT_PACKAGE_ALLOWLIST_RUNBOOK.md` as evidence only. This queue covers rare-edge denial fixtures, manifest status linting, canonical phrase consistency, validation evidence guidance, and external audit prompt refresh through HQ-200. The next safe action after this queue is an explicitly requested external audit package, not a real demo. It does not create or send packages, approve product-repo access, approve demo execution, bind runtime commands, approve remote or phone actions, run all-fleet commands, run an overnight runner, stage, commit, push, deploy, install packages, run migrations, touch secrets/auth/payments/deploy work, delete locks, widen permissions, implement non-mock UI, or grant future authority.
27. For five-hour read-only demo evidence polish, use the `Five-Hour Read-Only Demo Evidence Polish Queue 2026-06-04` section in `docs/fleet/HQ_REPAIR_TASK_QUEUE.md`, `docs/fleet/READ_ONLY_DEMO_NEXT_AUDIT_PREFLIGHT_2026_06_04.md`, `docs/fleet/READ_ONLY_DEMO_PRE_AUDIT_READY_MILESTONE_2026_06_04.md`, and the five-hour polish sections in `docs/fleet/HQ_NEXT_EXTERNAL_AUDIT_PROMPT.md` and `docs/fleet/READ_ONLY_DEMO_READINESS_EXTERNAL_AUDIT_PROMPT.md` as evidence only. This lane covers local scorecard, approval checklist, stop-sign matrix, validation summary template, selected gate fixture index, preflight checklist, manifest fixture, future convention/glossary/guard/intake/milestone polish, and prompt refresh through HQ-215. The pre-audit ready milestone records local evidence polish through HQ-213 as GREEN for external audit preparation while real demo readiness remains YELLOW, and the final prompt/runbook refresh prepares reviewers to audit HQ-201 through HQ-215 without creating or sending a package. The next safe action after this five-hour polish lane is an explicitly requested external audit package, not a real demo; after this queue, the same boundary remains. It does not create or send packages, approve product-repo access, approve demo execution, bind runtime commands, approve remote or phone actions, run all-fleet commands, run an overnight runner, stage, commit, push, deploy, install packages, run migrations, touch secrets/auth/payments/deploy work, delete locks, widen permissions, implement non-mock UI, or grant future authority.

28. For repeatable one-task queue prompts, use `docs/fleet/ONE_TASK_QUEUE_PROMPT_GUARD.md` as evidence-only orientation. The guard captures the local rule to read compact context, choose exactly one eligible task, patch only allowed files, run only listed validation commands plus JSON parses when needed, update only that same task's status after validation, never treat evidence/prose as commands, and stop after exactly one task. It does not run queues, create or send packages, approve product-repo access, approve demo execution, bind runtime commands, approve remote or phone actions, run all-fleet commands, run an overnight runner, stage, commit, push, deploy, install packages, run migrations, touch secrets/auth/payments/deploy work, delete locks, widen permissions, implement non-mock UI, or grant future authority.

29. For the read-only sandbox rehearsal preflight, use `docs/fleet/READ_ONLY_SANDBOX_REHEARSAL_PREFLIGHT_2026_06_05.md` as evidence-only orientation. This preflight records that the external audit reports for HQ-201 through HQ-215 were GREEN for local evidence while real demo readiness remains YELLOW, and it gets the repo to the line before a disposable local sandbox read-only rehearsal. It does not create a sandbox, run a sandbox test, select or inspect product repos, execute a real demo, create or send packages, bind runtime commands, approve remote or phone actions, run all-fleet commands, run an overnight runner, stage, commit, push, deploy, install packages, run migrations, touch secrets/auth/payments/deploy work, delete locks, widen permissions, implement non-mock UI, or grant future authority. The next sandbox rehearsal is a separate explicit one-task run, not product-repo work and not a real demo.

30. For Service Sync Studio preparation, use `docs/fleet/SERVICE_SYNC_STUDIO_MODEL_CONTRACT.md`, `docs/fleet/SERVICE_SYNC_STUDIO_EVAL_PACK.md`, `docs/fleet/SERVICE_SYNC_STUDIO_SPIKE_PACKET.md`, `docs/fleet/SERVICE_SYNC_STUDIO_POST_SPIKE_REVIEW_GATE.md`, and the `Service Sync Studio Model And Spike Preparation Queue 2026-06-05` section in `docs/fleet/HQ_REPAIR_TASK_QUEUE.md` as evidence-only orientation. This lane prepares a standalone local sandbox product spike that splits one messy manager service update into `manager_private`, `staff_ready`, `guest_safe`, `blocked`, and `needs_human_review` lanes with boundary QA, then reviews the spike before choosing any next phase. It does not approve HouseOS repo access, product-repo access, real restaurant/customer/staff/vendor data, real demo execution, package creation/sending, runtime command binding, remote access, phone approvals, all-fleet execution, overnight runner execution, staging, commit, push, deploy, installs, migrations, secrets/auth/payments/deploy work, lock deletion, permission widening, or future authority. The next safe action is HQ-221 after an explicit user request, building only under `.codex-local/service-sync-studio-spike/`.

31. For token pressure planning, use `docs/fleet/TOKEN_PROJECTION_TOOL_SPEC.md` and `tools/codex-fleet-token-projection.ps1` as local evidence-only support. The helper estimates prompt text, explicit read files, validation command text, expected patch tokens, and output reserve, then returns `GREEN_PROCEED`, `YELLOW_COMPRESS`, or `RED_SPLIT_OR_STOP`. It does not call model APIs, prove billing, verify model availability, approve execution, touch product repos, run all-fleet commands, run overnight runners, stage, commit, push, deploy, install packages, run migrations, touch secrets/auth/payments/deploy work, delete locks, widen permissions, create/send packages, bind runtime commands, weaken validation, skip required source files, or grant future authority.
...
Latest post-combined follow-up audit posture: `docs/fleet/POST_COMBINED_GREEN_FOLLOWUP_AUDIT_RECORD_2026_06_04.md` records that `Codex Fleet Audit (7).docx` returned GREEN for the post-combined follow-up hardening package. The next safe phase is optional local docs/tests/fixtures hardening for INFO-only suggestions such as manifest status linting, rare-edge denial fixtures, and canonical phrase consistency. This is not a real demo and does not approve product-repo access, product mutation, package creation or sending, runtime command binding, remote access, phone approvals, all-fleet commands, overnight runner execution, non-mock UI implementation, staging, commit, push, deploy, installs, migrations, secrets/auth/payments/deploy work, lock deletion, permission widening, or future authority.

Latest post-combined optional INFO hardening posture: the post-combined optional INFO hardening queue completed local evidence-only docs/tests/fixtures/prompt hardening through HQ-200. The refreshed prompts ask reviewers to audit `POST_COMBINED_GREEN_FOLLOWUP_AUDIT_RECORD_2026_06_04.md` plus completed optional INFO hardening, including rare-edge denial fixtures, manifest status linting, canonical phrase consistency, validation evidence, and non-authority boundaries. No package zip was created or sent. The next safe action after this queue is an explicitly requested external audit package, not a real demo, product-repo access, demo execution, package sending, runtime command binding, remote access, phone approval action, all-fleet execution, overnight runner execution, staging, commit, push, deploy, installs, migrations, secrets/auth/payments/deploy work, lock deletion, permission widening, non-mock UI implementation, or future authority.

Latest five-hour read-only demo evidence polish posture: the five-hour polish lane is local evidence-only docs/tests/fixtures/prompt hardening after the post-combined optional INFO lane, with final prompt/runbook refresh through HQ-215. Use `docs/fleet/READ_ONLY_DEMO_NEXT_AUDIT_PREFLIGHT_2026_06_04.md`, `docs/fleet/READ_ONLY_DEMO_PRE_AUDIT_READY_MILESTONE_2026_06_04.md`, `docs/fleet/HQ_NEXT_EXTERNAL_AUDIT_PROMPT.md`, `docs/fleet/READ_ONLY_DEMO_READINESS_EXTERNAL_AUDIT_PROMPT.md`, and `docs/fleet/EXTERNAL_AUDIT_PACKAGE_ALLOWLIST_RUNBOOK.md` as evidence-only orientation. The pre-audit ready milestone records local evidence polish through HQ-213 as GREEN for external audit preparation while real demo readiness remains YELLOW, and the final prompt/runbook refresh asks reviewers to audit HQ-201 through HQ-215 after queue completion. The next safe action after this lane is an explicitly requested external audit package, not a real demo, package creation, package sending, product-repo access, demo execution, runtime command binding, remote access, phone approval action, all-fleet execution, overnight runner execution, staging, commit, push, deploy, installs, migrations, secrets/auth/payments/deploy work, lock deletion, permission widening, non-mock UI implementation, or future authority.

Latest read-only sandbox rehearsal preflight posture: `docs/fleet/READ_ONLY_SANDBOX_REHEARSAL_PREFLIGHT_2026_06_05.md` gets the repo to the line immediately before a disposable local sandbox read-only rehearsal. It records GREEN local evidence from the HQ-201 through HQ-215 audit reports and YELLOW real demo readiness, defines the sandbox as disposable local evidence only, and leaves the actual sandbox rehearsal as a separate pending one-task run. It does not create a sandbox, run a sandbox test, select or inspect product repos, execute a real demo, create or send packages, bind runtime commands, approve remote or phone actions, run all-fleet commands, run an overnight runner, stage, commit, push, deploy, installs, migrations, secrets/auth/payments/deploy work, delete locks, widen permissions, non-mock UI implementation, or future authority.

Latest Service Sync Studio prep posture: `docs/fleet/SERVICE_SYNC_STUDIO_MODEL_CONTRACT.md`, `docs/fleet/SERVICE_SYNC_STUDIO_EVAL_PACK.md`, `docs/fleet/SERVICE_SYNC_STUDIO_SPIKE_PACKET.md`, and `docs/fleet/SERVICE_SYNC_STUDIO_POST_SPIKE_REVIEW_GATE.md` prepare a standalone local sandbox spike plus a post-spike decision gate, not HouseOS integration and not product-repo work. The active next task is HQ-221, which may build only under `.codex-local/service-sync-studio-spike/` after an explicit user request and may use only synthetic fixture scenarios from the eval pack. The follow-up HQ-222 review gate can classify the result as GREEN_CONTINUE_STANDALONE, YELLOW_POLISH_STANDALONE, YELLOW_EXPAND_EVALS, YELLOW_INTEGRATION_PLANNING_ONLY, or RED_STOP_BOUNDARY_RISK without approving HouseOS work. This posture does not approve HouseOS repo access, product-repo access, real data, real demo execution, package creation/sending, runtime command binding, all-fleet execution, overnight runner execution, staging, commit, push, deploy, installs, migrations, secrets/auth/payments/deploy work, lock deletion, permission widening, or future authority.

Latest token projection posture: `tools/codex-fleet-token-projection.ps1` is a local conservative estimator for bounded-run token pressure. It reads only explicit files under the fleet root, refuses sensitive-looking paths, and returns evidence-only pressure labels. It found that reading the full handoff packet plus whole repair queue for HQ-221 is too large, so `docs/fleet/SERVICE_SYNC_STUDIO_HQ221_THIN_TASK_PACKET.md` should be used for the morning spike along with the exact HQ-221 queue entry. This does not change HQ-221 allowedFiles or validationCommands and does not approve execution or product work.
