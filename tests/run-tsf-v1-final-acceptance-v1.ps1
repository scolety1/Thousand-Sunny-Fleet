...
$semanticHotfixAllowedPattern = '^(docs/hq/tsf_v1_general_result_intent_fidelity_hotfix_v1_20260722/.*|fleet/control/(admission-decision|general-result-v2|original-operator-intent|result-envelope|scope-transformation|task-completion-contract)\.schema\.v1\.json|tools/(Invoke-TsfCodexAppServerForeground|New-TsfProjectMainBotMissionDraft|TsfSemanticIntegrity)\.ps1|tests/(Test-TsfGeneralResultIntentIntegrityV1|Test-TsfGeneralResultKernelVerifierV1|run-minimum-viable-kernel-tests|run-tsf-hq-dispatch-tim-relay-validation-v1)\.ps1|tests/(test-tsf-hq-dispatch-intent-authorization-v1|test-tsf-hq-dispatch-route-preview-v1|test-tsf-hq-dispatch-tim-relay-http-v1)\.mjs|tests/fixtures/fleet/project-main-bot/role_aware_lifecycle/requests/overnight-safe\.request\.json)$'
$unexpectedPaths = @($changedPaths | Where-Object { $_.Replace('\', '/') -notmatch $allowedPattern -and $_.Replace('\', '/') -notmatch $hotfixAllowedPattern -and $_.Replace('\', '/') -notmatch $semanticHotfixAllowedPattern })
