...
$expectedText='TSF_HQ_DISPATCH_READ_ONLY_GREEN'
...
$mission=[pscustomobject]@{
    mission_id=$missionId;mission_revision=1;repo_path=$root;expected_artifacts=@('fleet/control/policy-manifest.v1.json');allowed_writes=@();forbidden_writes=@()
...
function Invoke-ExactVerifierCase([string]$Name,[scriptblock]$Mutate){
    ...
    $exact=[pscustomobject]@{mission_id=$missionId;mission_revision=1;run_id=$runId;result_id=$runId;thread_id='thread-exact';turn_id='turn-exact';adapter_result_path=$adapterPath;adapter_result_sha256=$adapterHash;expected_response_sha256=$expectedHash;observed_response_sha256=[string]$adapter.observed_response_sha256;transport_success=$true;exact_match=$true;semantic_success=$true}
...
try{
    ...
    $wrong=Invoke-ExactVerifierCase 'wrong-response' {param($a,$w)if($null-eq$w){$a.final_response='WRONG_NONEMPTY';$a.observed_response_sha256=$expectedHash}}
    Assert-Case ($wrong.verdict-eq'RED') 'verifier rejects a wrong nonempty response even when producer flags claim success'
