...
function New-Mission([string]$Name){
    $m=Copy-Value $template;$m.mission_id="synthetic-self-hosted-$Name-$nonce";$m.mission_revision=1;$m.parent_mission_id=$null;$m.original_request="Fixture-only lifecycle terminal result case $Name";$m.normalized_goal="Return fixture-only terminal behavior for $Name without launching a worker.";$m.created_at=[datetimeoffset]::UtcNow.ToString('o');$m.expires_at=[datetimeoffset]::UtcNow.AddHours(2).ToString('o');$m.repository_allowlist=@($repo);$m.branch_worktree_policy.expected_branch=$branch;$m.branch_worktree_policy.expected_worktree=$repo;$m.branch_worktree_policy.starting_head=$head;$m.policy.policy_commit=$head;$m.policy.fingerprint=[string]$fingerprint.fingerprint;$m
}
