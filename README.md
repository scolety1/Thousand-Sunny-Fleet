...
`fleet-autopilot-policy.ps1` is the Phase 9 limited business autopilot gate. It validates `AUTOPILOT_POLICY.md` and `AUTOPILOT_APPROVAL.md`, requires explicit rules for spending limits, customer-data handling, and escalation, writes a Markdown report, JSON report, and audit log under `.codex-local/audit/`, fails closed on dirty ships unless `-IncludeDirty` is explicitly passed, and never spends money, deploys, emails customers, changes auth/payments, edits legal text, or touches customer data.
