...
.\fleet-reference-brief.ps1 -Project ShiftLedger -Write
.\harbor-master.ps1
...
`fleet-reference-brief.ps1` previews, validates, or writes `docs/codex/REFERENCE_BRIEF.md`. Hospitality Studio ships use this as the creative direction gate: it records the surface type, reference-quality targets, first-screen rules, forbidden patterns, and the "under 30 seconds without overload" acceptance lens. Nami, Simon, Robin, and the launch gate read it so the fleet does not forget the restaurant-grade restraint target during long runs.
...
`fleet-autopilot-policy.ps1` is the Phase 9 limited business autopilot gate. It validates `AUTOPILOT_POLICY.md` and `AUTOPILOT_APPROVAL.md`, requires explicit rules for spending limits, customer-data handling, escalation, and concrete safe automatic lanes, writes a Markdown report, JSON report, and audit log under `.codex-local/audit/`, fails closed on dirty ships unless `-IncludeDirty` is explicitly passed, and never spends money, deploys, emails customers, changes auth/payments, edits legal text, or touches customer data. Approved autopilot lanes must be listed in the policy safe-lane section and must not contain human-approval-only actions. Use `-ExcludeProject` for docked or personal ships that should not participate in a selected policy run; if the selected/excluded set leaves no ships, the gate fails closed instead of approving an empty autopilot run.

`franky-formula-review.ps1` is the Phase 10 specialist reviewer layer for analytical and formula-heavy ships. Franky is deterministic and checks `FORMULA_SPEC.md`, `FIXTURE_TEST_PLAN.md`, formula-oriented tests, analytical number provenance, and calibration visibility before formula work can pretend to be insight. Use `-Template` in a repo to scaffold the required formula evidence files before analytical work starts. Normal reviews write both `FRANKY_FORMULA_REVIEW.md` and `FRANKY_FORMULA_REVIEW.json`, including required actions and a repair-task draft when RED formula blockers appear, and the checkpoint loop stages both artifacts. The checkpoint loop can run Franky with `-FrankyEvery 1`, and analytical phases auto-run it before the final checkpoint.
...
`fleet-supervisor.ps1` writes `out/fleet-supervisor.md` and `out/fleet-overnight-digest.md`, and can stay open as an all-day watchdog. It classifies each ship as progressing, ready, idle, blocked, looping, over budget, or `BLOCKED_STAGING`; shows active work pack, Simon improvement score, information-staging status, run lock state, task budgets, and safe recommendations; and gives safe restart guidance without deleting locks or killing active work. Add `-AutoSafeStop` during unattended runs to create safe-stop requests for over-budget, looping-quality, idle-running, blocked-review, or blocked-staging ships so they pause at the next task boundary instead of churning overnight.

Add `-AutoRepair` when you want the supervisor to queue one high-priority repair task for clean, stopped ships in `BUDGET_STOP`, `LOOPING_QUALITY`, `BLOCKED_REVIEW`, or `BLOCKED_STAGING`. It prepends a small bounded task to `docs/codex/TASK_QUEUE.md`, records the action in `docs/codex/AUTO_REPAIR.md`, commits the repair task on that ship branch, and skips active or dirty ships. For `BLOCKED_STAGING`, the queued repair is docs/task-metadata first: complete `INFORMATION_STAGING.md` or add the missing `surface:*` / `First screen:` metadata before any launch. Add `-ClearSafeStopAfterRepair` only when you want that ship eligible for the next launcher run immediately after the repair task is queued.
...
For the information-staging loop, add `-QueueStagingRepairs` only when you want visual findings to create launch-gate-ready tasks in the ship's `docs/codex/TASK_QUEUE.md`. This queues only staged-information repair tasks from the `Information Staging Review`, de-duplicates exact task text, and writes them under a `Visual staging repair tasks` section so the next checkpoint loop can repair overloaded first screens without a human copying report text by hand. Queueing is skipped when the repo was already dirty before visual QA, so the report can still be inspected without mixing repair tasks into active user or ship work. Add `-ValidateQueuedStagingRepairs` with `-QueueStagingRepairs` when you also want the fleet launch gate to verify the queued tasks are immediately launchable; the visual summary records whether the staging repair launch gate accepted the queued work.

`tests\run-fleet-tests.ps1` runs deterministic fleet tests without touching real ships. It generates disposable fixture repos under `.codex-local/fixtures/`, validates parsing/config/guardrail helpers, and removes fixtures when it finishes unless `-KeepFixtures` is passed. For a live smoke run that actually calls Codex, generate fixtures with `.\tests\new-fixture-ships.ps1 -Force -UseRealModels`; this uses `gpt-5.4-mini` first so the smoke test is supported and low-cost.
...
`docs/codex/INFORMATION_STAGING.md` is the progressive-disclosure contract. Nami, Simon, and Robin use it to keep pretty demos from becoming walls of everything: the first screen must show the primary job, secondary actions stay obvious but quieter, detail/internal information opens through buttons, tabs, accordions, drawers, cards, detail views, or staff/internal mode. The launch gate requires the first-screen contract fields to be filled: `First screen job`, `Primary content`, `Secondary actions`, `Detail content`, `Not visible at first`, and `How deeper information opens`. Restaurant ships should separate a customer-facing restaurant example from the working operations tool; product ships such as EasyLife or CursorPets should separate the public/product demo surface from the actual app surface.

`docs/codex/REFERENCE_BRIEF.md` is the taste contract for Hospitality Studio ships. It is not a copy-the-reference command; it says what qualities to borrow, what not to copy, what belongs on the first screen, and which patterns are forbidden. Use it when the user names inspiration sites or says a demo should feel more refined, editorial, premium, restaurant-grade, or less overwhelming.

`fleet-product-dashboard.ps1` also reports each ship's information-staging status. The `Staging` column shows whether the ship has a complete `INFORMATION_STAGING.md` contract and whether the next visible/product task includes exactly one `surface:*` tag plus a concrete `First screen: ...` field. Ships with missing or incomplete staging are marked for `fix staging contract` before launch, which gives the captain one control-room view of overloaded-first-screen risk.
...
Phase 10 specialist reviewers give the Fleet domain-specific stoppers. The first reviewer is Franky Formula Review: formula/model work must show a spec, fixtures, expected outputs, tests, provenance, and calibration caveats before the loop treats analytical output as trustworthy. Franky can also create the starting evidence templates, emits structured JSON so future dashboards can track formula gates without scraping prose, and gives the repair loop concrete next actions instead of a vague math failure.
