...
`test-fleet-harness.ps1` is the deterministic control-room self-test. It parses core scripts, verifies selected launch inclusion/exclusion, checks that launch validation rejects unexpected ships, verifies night-report dry-run filtering, and validates the selected checkpoint-loop configs unless `-SkipProjectValidation` is passed. It writes `out/fleet-harness-test.md`.
