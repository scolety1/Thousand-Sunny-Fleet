...
Do not mutate these project folders in autonomous completion runs yet. The next safe step is for Tim to either provide the canonical git repo path for each project or explicitly approve converting the intended folder into a git-backed project.

Suggested prompt:

~~~text
Use TSF's blocked project repo audit.
For PromptLab, FitTrack, Personal Site / Portfolio, and RepoTriage, prepare git-bootstrap work orders only.
Do not run git init, mutate project files, install packages, push, deploy, migrate, touch secrets, or inspect unrelated projects.
~~~
